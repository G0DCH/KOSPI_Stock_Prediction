...
def Normalize(dataList, stockCode):
    ...
    for window in tqdm(dataList):
        ...
        normalizedWindow['종목코드'] = float(hashCode) / float(pivotCode) - 1
...
def Run():
    ...
    x_train_name = os.path.join(path, 'X_Train')
    y_train_name = os.path.join(path, 'Y_Train')
    x_test_name = os.path.join(path, 'X_Test')
    y_test_name = os.path.join(path, 'Y_Test')
    pivot_name = os.path.join(path, 'pivot')
